            ...
            <div className='md:hidden flex'>
                ...
                <div className={`absolute left-0 ${showNav ? '' : '-translate-x-full'}  flex justify-start vs:pl-10 pl-2 xs:p-5 p-2 xs:top-[81px] vs:top-[77px] top-[68px] bg-[#ffe] w-[100%] min-h-svh transform transition-all duration-500 ease-in-out`}>
                    <ul className='flex flex-col vs:gap-8 gap-2 w-full'>
                        {navLinks.map((navlink, index) => (
                            <li key={index} className='font-semibold hover:text-[goldenrod] hover:font-bold border-b-[goldenrod] border-b-2 py-2 px-1'>
                                {index === navLinks.length - 1 ?  (<NavLink to={navlink.link}>
                                    {navlink.name}
                                </NavLink>) : (<a href={navlink.link}>{navlink.name}</a>)}
                            </li>
                        ))}
                        <li className='font-semibold hover:text-[goldenrod] hover:font-bold border-b-[goldenrod] border-b-2 py-2 px-1'>
                            <a href="#contact">
                                Contact
                            </a>
                        </li>
